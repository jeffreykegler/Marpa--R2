...
language standard and recasting it so every rule takes into account
whitespace.
...
But long before you finish, I expect you will come to believe
that the "unified" approach is simply not going to work.
The authors of C
...
=head1 Reuniting the two levels
...
The programmer needs to decide when to use the "C<~>" operator
and when to use the "C<::=>" operator,
but the choice will usually be intuitive:
If you want Marpa to "do what I mean" with whitespace, you use the
"C<::=>" operator.
If you want Marpa to do exactly whay you say on a character-by-character basis,
...
Significant in the scannerless interface are the tokens
at the boundary between G0 and G1.
These are the top-level undiscarded symbols in G0,
which go on to become the terminals in G1.
These are called "lexemes".

To find G0's lexemes, Marpa looks for symbols which are on
the LHS of a G0 rule, but not on the RHS of any G0 rule.
To determine G1's idea of what the lexemes are,
Marpa looks for symbols on the RHS of at least one G1 rule,
but not on the LHS of any G1 rule.
If G0 and G1 have different ideas about the set of lexemes,
...
Those in discard rules are thrown away, and the rest are passed
as tokens to the G1 grammar.
Note that match is longest TOKENS.
Marpa does ambiguous parsing and ambiguous lexing.
...
before changing to the dual grammr implementation.
