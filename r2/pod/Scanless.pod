...
convince himself by taking the BNF from your favorite
...
that the "unified" approach is simply not practical
and that the authors of C clearly made the right choice in
relegating lexical issues to their own section,
and so that they could ignore them in
most of their language description.

=head1 Reuniting the two levels

The scanless interface, I hope,
reunites and even to an extent
interweaves the two levels of description,
but in a way that preserves the advantages of both.
