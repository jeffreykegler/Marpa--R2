...
=head1 Nulling variants

The "breakpoint" of a cycle occurs when it loops
back to the same
rule.
This raises the question of whether nulling variants
of a rule are "different" or "the same".
Marpa's behavior here is only partially specified
at this point.
Marpa guarantees that it will produce a parse for
at least one of the nulling variants.
Which one is unspecified.
Also unspecified is
how many other nulling variants Marpa will produce
parses for.
Marpa's current behavior depends
on internals of the implementation,
and is subject to change.

It is not clear whether the "right" behavior
would be to return all nulling variants,
or just one.
And if just one, it is also not clear which
one.
It seems more clear that
the quality of the present implementation
is fully adequate to the present interest.
