...
named argument
is defined,
a symbol's null value is arbitrary.
"Arbitrary" here means
that the value can not be relied
on,
even to stay the same for
the same symbol in the same parse.

Marpa optimizes for arbitrary null values,
and in casess where null values are not used for anything,
applications should prefer arbitrary null values.
