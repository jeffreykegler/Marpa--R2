...
Each rule declaraton consists of a left hand side (LHS),
followed by the declaration operator ("C<::=>")
and a right hand side (RHS).
The left and right hand sides
are described in detail below,
in the discussion of each rule by type.
...
A simple rule,
with only a series of RHS symbols
on their RHS,
is treated as a
...
The prioritization may be trivial, consisting only of a single
...
Adverbs are allowed for quantified productions,
...
The C<assoc> adverb is only valid in a prioritized production.
...
=head1 How Marpa precedence works
