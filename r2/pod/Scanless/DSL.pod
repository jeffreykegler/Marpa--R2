...
The C<action> adverb specifies the semantics for a rule
or lexeme.
The default value of the semantics for a rule is a Perl C<undef>.
...
Word actions are not allowed for for lexemes.
...
The value of the semantics is an array.
...
If the action of a lexeme is C<::dwim>, the value of the lexeme
will be its token value if the lexeme is not blessed.
If the lexeme is blessed, its value will be an array of length
one, the only element of which is the token value.

For a rule, the C<::dwim> action
...
If the action is for a
rule that is blessed, C<::dwim> is equivalent to C<::array>.

=item *

If the action is for a sequence rule, C<::dwim> is equivalent to C<::array>.

=item *

If the action is for a rule with two or more children,
C<::dwim> is equivalent to C<::array>.

=item *

If the action is for a rule with one child,
C<::dwim> is equivalent to C<::first>.

=item *

If the action is for a rule with no children,
C<::dwim> is equivalent to C<::undef>.
...
If the value of an action adverb begins with a left square bracket,
it is an array descriptor.
An array descriptor is a comma separated list of zero or more array
descriptor items.
The value of the lexeme or rule will be an array, with contents
as determined by the array descriptor items.
The lists will be placed in the array in the order
specified.
If no array descriptor items are specified, the value
of the lexeme or rule will be an empty array.

Currently there are only two array descriptor items.
The C<range> array descriptor item will put a list two elements
into the array indicates the location in the input stream
of the rule or lexeme.
The first element will be the start position,
and the second will be the end position.
...
For a lexeme, 
the C<value> array descriptor item will be a single element --
...
If the adverb value is a Perl word
...
named argument|Marpa::R2::Scanless::G/"bless_package">.

If the value begins with a double colon ("C<::>"), it is a
reserved value.
A bless value of C<::undef> means no blessing, and this is the
default for both rules and lexemes.
If any rule or lexeme of a SLIF grammar has
a bless value other than C<::undef>,
a C<bless_package> is required,
and failure to specify one results in
a fatal error.

The bless adverb for
rules allows one additional reserved value: C<::lhs>.
