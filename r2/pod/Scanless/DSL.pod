...
rule adverbs.
The default pseudo-rule must be a G1 rule,
and it changes the defaults only for other G1 rules.
...
One of its intended uses to is the
creation of an AST,
in a form that is convenient to traverse and evaluate,
but that avoids trivial and useless nodes,

If the action of a lexeme is C<::dwim>
and the lexeme is not blessed,
the value of the lexeme
will be its token value.
If the lexeme is blessed,
its value will be an array of length one,
the only element of which is the token value.
...
The C<range> array descriptor item will put a list
into the array that indicates the location in the input stream
...
The value of C<bless> adverb is called the B<blessing>.
...
No blessing is the default for both rules and lexemes.
...
a class whose name is based on the name of the rule.
