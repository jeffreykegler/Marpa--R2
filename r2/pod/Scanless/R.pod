...
The second argument is a position in the input string
...
A negative start location is a location counted
from the end, so that -1 means start at the last
character, -2 at the second to last, etc.
The default start position is zero.

A negative length means that internal scanning
should pause at a location counted from the end,
so that -1 means continue to the end of the string,
-2 means pause before the last character of the string,
etc.  The default length is -1.
...
returns the start location and
length of the most recent match.
The length is in parse locations,
so that the end location will
always be the start location plus the length.
...
Given a start location and its length, returns a substring of the input
string.
The length is in locations,
so that the end location is always start location
plus length.
