...
Internal scanning ends when one of the following occurs:
...
A scan that reaches EOS
is not considered a pause.
...
the pause (it may not be unique).
The pause span will be the span of the input stream
occupied by that lexeme.
...
An abend is not considered a pause.
...
For this to be the case, both lexemes must be
acceptable to the G1 grammar,
and both must have
same lexeme priority.
