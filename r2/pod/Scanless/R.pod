...
-2 the position before the second to last character of a string, etc.
...
The EOS location does not point to any character.

In this interface, when the current input stream location
...
The current location will never be past EOS.
...
As already stated, the current location will never advance past EOS.
...
This default is the result of considerable experience
...
stream before which internal scanning ended.
...
The C<read()> method performs internal scanning,
L<as described
