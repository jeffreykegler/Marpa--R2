...
as specified by C<$start> and C<$length>.
...
It returns the line and column of that location.
...
A pause is not always caused by a lexeme -- it may be
caused by the length argument of an internal scanning method.
If the most recent pause was not caused by a lexeme,
the pause lexeme is undefined.
C<pause_lexeme()> returns a Perl C<undef> if
the pause lexeme is undefined.
