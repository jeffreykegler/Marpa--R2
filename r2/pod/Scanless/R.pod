...
After the C<read()> method, the current location will indicate how far
...
C<read()> will pause when the current location is C<$start+$length>.

=item *

If a negative C<$length> argument was specified to C<read()>,
C<read()> will pause when the current location is
...
using the C<pause> adverb.

=item *

An abend
in the C<read()> method causes the current location to
