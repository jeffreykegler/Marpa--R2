...
If a rule was specified using the BNF (Stuifzand) interface,
it will be given as specified in the BNF source.
To see the rule in its rewritten form,
use the 
L<the C<rule()> method|/"rule()">.
Situations where
...
If a rule was specified using the BNF (Stuifzand) interface,
it may be in a rewritten form.
To see BNF interface rules in their
original form,
use the 
L<the C<bnf_rule()> method|/"bnf_rule()">.
Situations where
Rule ID's are encountered include
callbacks and
use of L<the progress method|Marpa::R2::Progress/progress()>.
