...
If a grammar with infinite loops is ambiguous,
it can produce valid parses as well as infinite ones.
Marpa will parse with infinite loop grammars,
and Marpa's evaluator will return the values from the grammar's
non-infinite parses.
