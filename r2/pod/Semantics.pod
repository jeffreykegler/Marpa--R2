...
While it is possible to modify the values pointed to
by a reference, the effect will be global,
this is deprecated as bad practice.
Reference actions, and their referents, should be treated
as read-only.
...
to start with C<$empty_hash> and add contents.
C<$empty_hash> points to a single hash object,
which will shared by all nodes, with all nodes seeing
each other's changes.
Worse, all Marpa parsers which use the same C<My_Nodes>
namespace will also share the same hash object.
