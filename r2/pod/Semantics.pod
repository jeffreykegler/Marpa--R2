...
=item * A reference to a Perl closure.
This must be in the form of a reference whose type,
as returned by C<ref>, is C<CODE>.
When this is associated with a rule,
the Perl closure
is called a B<rule evaluation closure>.
The closure will be called to produce the value of
the action.

=item * A scalar constant, that is one whose
type, as returned by C<ref>, is C<SCALAR>.
The constant will be the value of the action.

=item * A reference to an array,
that is, a reference whose
type, as returned by C<ref>, is C<ARRAY>.
The constant will be a reference to that array.

=item * A reference to a hash,
that is, a reference whose
type, as returned by C<ref>, is C<HASH>.
The constant will be a reference to that hash.

=item * A special, reserved, action name,
as described under
L<Reserved Action Names|/"Tree Traversal Phase">.
...
To allow Marpa to help you to spot non-existent values,
