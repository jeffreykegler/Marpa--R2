...
=head2 Rank of different rules with same rank is now arbitrary

In ranking rules,
if two different rule instances are for different
rules, but the two rules have the same rank,
their relative ranking is now arbitrary.
...
and an alternate
and more cumbersome way to mark terminals is not worth cluttering
