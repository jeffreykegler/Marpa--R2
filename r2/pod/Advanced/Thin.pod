...
but is unusual is having a variety of soft failures.
Following the general pattern, C<precompute()> returns
a Perl C<undef> for all of these soft failures.
To tell them apart, the error methods must be used.
...
C<marpa_g_ref()>
and C<marpa_g_unref()>
are not included of the Marpa thin interface.
...
follow the general pattern, as describe L<in another
