...
In the general pattern,
return values from Libmarpa method
will always either
belong to
one of Libmarpa's numbered classes,
or be C language C<int>'s.
The corresponding
general pattern Marpa thin method
will return the integer as
a numeric Perl scalar,
with one exception.
When a Libmarpa method returns -1,
its corresponding general pattern Marpa thin method
will return a Perl C<undef>.

Note that booleans do not receive special recognition.
Even when a Libmarpa interface treats an
argument as a booleans, the corresponding
general pattern Marpa thin method will treat
its arguments as if they were any other numeric,
without regard to their "Perl truth value".
