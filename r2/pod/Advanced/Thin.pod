...
Methods that follow
the general pattern are not specifically mentioned.
Methods that are
exceptions
to the general pattern
are described in detail.
Those
Libmarpa methods to which
...
the corresponding Marpa thin interface method
...
corresponding to
each of Libmarpa major classes.
...
the Marpa thin interface to to adapt Libmarpa's
...
these must be able to set the Marpa thin
...
When the last error was a Libmarpa error,
...
Error descriptions differ from error names.
...
Error descriptions 
may context text relating, not just to the error code,
...
The programmer can expect error codes and error names
to remain stable and can write code that relies
on the numeric value of the error codes
and the text of the error name.
Applications should rely on the text of an error description for
only one purpose: passing it on to a human user.
...
Error codes and error descriptions should only be considered
valid if they are queried immediately after a method returns,
and only if that method indicated that an error description
can be expected.
"Immediately" in this context means before another call to
another thin interface method, except the L<< C<error()>
method|"$g->error()" >>.
Indication of failure can be either a thrown exception,
or a return value.
Which return values indicate that there is an error code
is documented for each method.
For those methods following the general pattern,
for example,
an error description can be expected only if failure was
thrown or if the method returned a value equal to or
less than -2.
...
On a thin interface failure,
thrown or unthrown,
the method sets the error code to a Perl C<undef>.
It also sets the error description
to a string which describes the failure.

=item *

In a few cases,
as documented in the Libmarpa API,
Libmarpa set an "informational"
error code on success.
(In this context, "error code"
becomes something of a misnomer.)
An appropriate error description is also set.
These "informational" error codes
are solely intended as a convenience,
and the programmer should consider herself
free to ignore them.
Whenver a Libmarpa method sets an informational error code,
the code, the circumstances under which it is set,
and the associated return value,
...
On success,
if a method does not set an informational error code,
...
the general pattern exactly are not
...
In the previous example, C<start_symbol_set()>
will be in
...
Zero or more non-self arguments follow this first time class argument.
The arguments of the corresponding general pattern
Marpa thin method will be
the same, converted from C types to Perl as described below.
...
C language C<int>'s are converted to Perl scalar integers.
Arguments which belong to Libmarpa's numbered 
classes (for example,
C<Marpa_Rule_ID> and C<Marpa_Symbol_ID>)
are also converted to Perl scalar integers.
...
In the general pattern,
the return values from Libmarpa method
will always either
belong to
...
it will return it as a numeric Perl scalar.
If the Libmarpa method returns -1,
its corresponding general pattern Marpa thin method
will return a Perl C<undef>.
For the treatment of other negative return values,
see the section on L<the general failure handling
pattern|"The general failure handling pattern">.

=head2 The general failure handling pattern

The general failure handling pattern is part
of the general pattern.
In addition there are several methods that do not
follow
the general pattern, but that do follow the
general failure handling pattern in part
or in whole.
These cases will be noted specifically in
descriptions of those methods
in this document.

General pattern methods consider failure to be
a return of -2 or less.
If the throw setting is 1,
the failure is thrown.
On unthrown failure, the return value
of the Libmarpa method will
also be returned by the Marpa thin
method as a numeric Perl scalar.
The Libmarpa error code
and a Marpa thin interface error name
and error description
will be available,
as described in L<the section
on error
methods|/"Error methods">.

In the general pattern,
if the Libmarpa method returns -1,
the corresponding Marpa thin method returns
a Perl C<undef>.
In the general pattern,
a return of -1 is NOT considered a failure.
...
methods follow the general failure
...
always follow the general failure handling pattern.
...
The first element is the string "C<MARPA_STEP_TOKEN>".
The second element is the token id, as returned
by the Libmarpa method C<marpa_v_token()>.
The third element is the token value, as returned
by the Libmarpa method C<marpa_v_token_value()>.
The fourth element is the token's stack location,
as returned
...
The first element is the string "C<MARPA_STEP_NULLING_SYMBOL>".
The second element is the token id, as returned
by the Libmarpa method C<marpa_v_token()>.
The third element is the token's stack location,
as returned
by the Libmarpa method C<marpa_v_token_arg_n()>.
...
The first element is the string "C<MARPA_STEP_RULE>".
The second element is the rule id, as returned
by the Libmarpa method C<marpa_v_rule_id()>.
The third element is the stack location of the
rule's argument 0,
as returned
by the Libmarpa method C<marpa_v_token_arg_0()>.
The fourth element is the stack location of the
rule's last argument,
as returned
by the Libmarpa method C<marpa_v_token_arg_n()>.

On failure,
C<step()> obeys the throw settings.
On unthrown failure,
...
captialization variants.
The string will usually be a description of the error.
On failure,
C<step()> also sets the error code and error description
in conformance to the general failure handling pattern.
...
Because they are folded into the thin interface's
C<< $v->step() >> method,
the thin interface does not have any methods
corresponding directly to the
step accessor macros.
