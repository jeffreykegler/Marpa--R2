...
a thin interface method, except the L<< C<error()>
method|"$g->error()" >>.
...
In the rest of this document, the phrase "sets
a Libmaqrpa error code" should be taken to mean
"sets
a Libmaqrpa error code,
together with an appropriate error description".
Here are the specifics on when an application can
expect and error code and/or error description to
have been set:
...
thrown or unthrown,
the method sets a Libmarpa error code.
...
In a few cases,
Libmarpa sets an Libmarpa error code
on success.
An error code that is the result of success
is called an information error code,
the phrase "error code" in this context
being something of a misnomer.
"Informational" error codes
are solely intended as a convenience,
and the programmer should consider herself
free to ignore them.
For every method that sets an information
error code,
the Libmarpa API document gives specifics,
including the name of the error code,
and the circumstances under which it is set.
...
A "cleared" error code is set to C<MARPA_ERR_NONE>.
Second, it may leave the error code as is.
