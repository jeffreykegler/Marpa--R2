...
are often not omitted from the Marpa thin interface.
Whenever an external Libmarpa method is omitted from
the thin interface,
that will be specificially stated in this document,
along with a reason for the omission.
...
in conformance to the general error handling pattern.
