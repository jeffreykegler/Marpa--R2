...
unspecified, unless stated in the descripton
...
Libmarpa's configuration objects
and methods are omitted from the Marpa
thin interface.
Their functions are handled by the thin
interface internally.
The Marpa thin interface has a configuration
variable:
...
as its one argument.
On success, it returns a Marpa thin recognzier object.
...
Ruby Slippers mode is OFF by default in the thin
interface -- the opposite of the default in the
