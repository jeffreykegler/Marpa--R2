...
a Libmarpa error code" should be taken as abbreviating
the phrase "sets the error code
to a Libmarpa error code,
and sets the error description to describe the error".
The phrase "undefs the error code"
should be taken as abbreviating
the phrase "sets the error code to a Perl C<undef>
and sets the error description to a message describing
the Marpa thin interface error".
The phrase "sets the error code" should be taken
to mean "sets a Libmarpa error code or undefs the error code".

Error codes and error descriptions should only be considered
valid
only if that method indicates that it sets the error code.
...
On success,
what a method will do with the error code and error
description is usually unspecified,
except that it will be one
...
together with an error descripton that indicates
there is no error.
...
When a Libmarpa method might set an informational
error code,
that is explicitly stated
in that method's description in
the Libmarpa API document.
Typically, informational error codes are
ignored.
...
was built.
On failure, C<new()> sets a Libmarpa error code.
...
On failure, C<event()> sets an error code.
...
On failure, C<rule_new()> sets the error code.
...
On failure, C<sequence_new()> sets the error code.
