...
my $self = {};
say "ast result = \n", Data::Dumper::Dumper(evaluate($self, ${$parse_result}));
...
sub evaluate {
	...
	return bless [ map { evaluate($self, $_) } @{$value} ], ref $value if Scalar::Util::reftype($value) eq 'ARRAY';
	return $value;
    }
    return [ map { evaluate($self, $_) } @{$value} ] if ref $value eq 'ARRAY';
    return $value;
} ## end sub evaluate
...
sub new {
    my ( $class, $self ) = @_;
    return bless { name => '' . $self }, $class if ref $self eq q{};
    return $self;
}
