...
sub do_character_class {
    shift;
    return Marpa::R2::Internal::Stuifzand::Symbol->new('[:character_class]');
}
...
$tracer->rule_new( "do_character_class" => "rhs_primary", "character_class" );
...
sub parse_rules {
    ...
    STEP: while (1) {
        ...
        if ( $type eq 'MARPA_STEP_RULE' ) {
            ...
            if ( $action eq 'do_character_class' ) {
                $stack[$arg_0] =
                    do_character_class( undef, @stack[ $arg_0 .. $arg_n ] );
                next STEP;
            }
