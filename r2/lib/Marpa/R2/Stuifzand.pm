...
sub set_mask_bits {
     $_->mask_bit_set() for @ { $_[0] };
}

sub mask {
    return map { $_->mask_bit() } @{ $_[0] };
}
...
## Date: Wed Nov  7 19:32:19 2012
$tracer->rule_new(
    "do_action" => "action",
    "kw_action", "op_arrow", "action_name"
);
$tracer->rule_new( undef, "action_name", "bare_name" );
$tracer->rule_new( undef, "action_name", "reserved_word" );
...
$tracer->rule_new(
    "do_masked_symbol_group" => "rhs_primary",
