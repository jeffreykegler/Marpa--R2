...
sub parse_rules {
        ...
        0 .. $thin_grammar->highest_rule_id() )
    {
        ...
        if ( $lhs eq 'symbol' ) {
            $actions_by_rule_id[$rule_id] = 'do_symbol';
            next RULE;
        }
        if ( $lhs eq 'symbol_name' and $rhs[0] eq 'bracketed_name' ) {
            $actions_by_rule_id[$rule_id] = 'do_bracketed_name';
            next RULE;
        }
