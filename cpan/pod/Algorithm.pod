...
Each confluence has two <inflows>,
...
A B<sentential form> is a sequence of zero or
more symbols from C<Syms>.
...
where C<beta> and C<beta> are sentential forms.

=back

Let C<alpha> and C<delta> be sentential forms.
We say the Earley item

  C<< [ [ <A> ::= <beta> . <gamma> ], origin, current ]  >>.

is B<correct> if and only if
we have all of the following:

   1. C<< <Start> => <alpha> <A> <delta> >>

   2. C<< <alpha> => W[0,(origin-1)] >>

   3. C<< <beta> => W[origin,(current-1)] >>

A proof that the Earley algorithm adds all correct Earley items,
and only correct Earley items to the Earley table.
can be found in Jay Earley's original paper,
