...
Then "end" character of the span may usually be calculated as
...
properly speaking,
their literals will have no first (start) or last (end) character.
...
and the same glade.
For this purpose, the C<undef> of a token alternative
is considered a rule ID.
The term B<symch> is shorthand for "symbolic choice".

For glade accessors, the accessor's value can be classified as
