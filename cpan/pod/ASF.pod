...
are of use.
The L<C<symbol_id()> glade method|/"symbol_d()"> is
...
The descriptions state whether the value will remain constant inside a glade,
will remain constant within a symch;
or can vary with each factoring.

Symches and factoring are terms which are
useful for some advanced applications.
For the purposes of this document,
the reader can regard the term "factoring" as a synonym
for "parse alternative".
A symch is a series of alternatives (factorings) which share the same rule id.
and the same glade.
...
The C<glade_literal()> method accepts no arguments.
...
Length may be zero.
The glade literal remains constant within a glade.
The C<glade_span()> method accepts no arguments.
...
The rule ID remains constant within a symch.
In fact by definition, within a glade, alternatives sharing the same rule ID are
a symch.
For this purpose, C<undef> is considered a rule ID.
...
It is a fatal error to call C<rh_length> for a token alternative.
The RHS length ID may vary for each factoring.
...
It is a fatal error to call C<rh_length> for a token alternative.
The RHS child values may vary for each factoring.
...
should be careful to check, not that the return value is a Perl true,
but that the return value is defined.

As a reminder, a symch is the set of alternatives which share the same
glade and the same rule ID.
For this purpose, C<undef> is considered a rule ID.
...
as a group.
More precisely, call the rule ID of the alternative at which the glade is
currently pointing, the "current rule ID".
The
C<next()> glade method guarantees that,
before
