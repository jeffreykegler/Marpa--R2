...
Of the symbols of the language,
one is distinguished as the B<start symbol>,
and a subset are designated as B<terminal symbols>.

A B<symbol string>, or simply B<string> where the meaning is clear,
is a sequence of the symbols of a grammar.
A symbol string is also called a B<sentential form>.
...
A B<language> is a set of sentences.
...
far more than a set of sentences.
...
Each token represents a terminal symbol of the grammar and has a B<value>.
A lexical analyzer is often called a B<lexer> or a B<scanner>,
and B<lexical analysis> is often called B<lexing> or B<scanning>.

The sequence of symbols represented by the sequence of tokens
becomes the B<input>
seen by the recognizer.
The input must be a sentence --
that is, it must be a sequence of terminals.
