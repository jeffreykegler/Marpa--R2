...
The glade hierarchy is a recursive structure whose nodes
are glades.
Every glade has the same internal structure:
...
A factor is the uphill end of a path which leads to a downglade.
A downglade is another glade.
...
ignore: 1
    ...
    [ 'factoring', $glade, $symch_ix, $rhs_ix1, $factoring_ix2, $rhs_ix2 ];
...
of that symch.
It can be seen as a duple C<E<lt>factoring_ix, rhs_ixE<gt>> where 
C<E<lt>factoring_ixE<gt>> is the index of a factoring
...
The RHS symbol instance with index C<rhs_ix> in the factor with index C<factoring_ix> is
said to be the RHS symbol at factor position C<E<lt>factoring_ix, rhs_ixE<gt>>.
