...
depends on the length of the string being factored, and can grow arbitrarily large.
...
an non-trivial symbolic choice is encountered.
(A trivial choice is one which has only one alternative, and therefore is not a choice
...
(Since it is the beginning of an ambiguous stretch, the two factors
...
and two factors at the beginning of the ambiguous stretch is
