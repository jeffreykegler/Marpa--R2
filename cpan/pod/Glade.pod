...
in other words, an ambiguity is not reported if it is downhill
...
To make ambiguity reporting as precise as possible,
...
so that, for each C<factor_ix>
so that C<< 0 <= rhs_ix < rhs_length >>,
where C<rhs_length> is the RHS length of the rule of interest.

Each downglade C<factor_ix, rhs_ix>
corresponds one-to-one to a G1 span,
that is, there is a total function
C<G1_start(factor_ix, rhs_ix)> from a downglade
to a G1 start location;
and there is a total function
...
A symch alignment is synced if its element share a common G1 length as well.
A symch alignment is unsynched if it is not synced.
...
Of special interest is the B<initial symch alignment>.
The initial symch alignment is the symch position all of whose RHS indexes
are 0.
Equivalently, it is the constant function C<ISP>, where C<ISP(factor_ix)=0>
that, for all C<factor_ix>
such that C<< 0 <= factor_ix < N >>.
It initial symch alignment is always a symch alignment.
...
of a symch alignment or,
as a special case,
if it is one past the last location of the G1 stream.
We say that a G1 span is B<anchored> if it its start
...
its start location will always be an anchor,
and the only anchored location in it
will be its start location.

Note that,
while multifactored stretch are aligned and anchored in terms of G1 locations,
and therefore in terms of input locations as well,
but they do not have to be aligned in terms of RHS indexes.
Very often, the
RHS indexes of
a multifactored stretch will have many different values.
