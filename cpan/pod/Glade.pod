...
If a glade is trivial and has a token symch,
then its only symch is a terminal
and that glade will be a terminal in the glade hierarchy.
...
a non-trivial symch is encountered.
...
but a specific subsequence within the RHS
which which is multiply factored -- a B<multifactored stretch>.
Marpa does this, because a RHS can be very long.
Marpa's sequence rules especially have long stretches where the symbols are in sync
...
Many of these details are restatements of what is said above
in more formal terms,
or formal statements of concepts that have been left to
the intuition.
Some find these details helpful,
other find them distracting.
Segregating them here serves the needs of both
kind of reader.
...
A rule symch is a set of factorings.
We need a way of identifying specific RHS symbols, or factors,
within that set of factorings.
Intuitively, a B<factor position> is a location within the factors of one of the factorings
of that symch.
It can be seen as a duple C<E<lt>factor_ix, rhs_ixE<gt>> where 
C<E<lt>factor_ixE<gt>> is the index of a factoring
within the rule symch,
and 
C<E<lt>rhs_ixE<gt>> is the index of one of the RHS symbols of the factoring.
Both the factoring index and the RHS index are zero-based.

Let I<N> be the number of factors in a rule symch.
A symch position is a set of I<N> factoring positions C<(factor_ix, rhs_ix)>,
such that there is exactly one element in the set for every factor.
More precisely,
for every I<<0 <= j < N>,
a symch position will have
<< j = factor_ix >> is exactly one of the pairs.
Alternatively, a symch position can be thought of as
a function from the factor indexes to the RHS indexes.
...
have a RHS index of 0.
That is, the initial symch position is the set of I<N> factoring positions.
for every I<<0 <= j < N>,
<< (j, 0) >> is one of the pairs.
Equivalently, it is the constant function C<ISP>, where C<ISP(i)=0> for all factoring
indexes C<i>.

The RHS symbol instance with index C<rhs_ix> in the factor with index C<factor_ix> is
said to be the RHS symbol at factor position C<E<lt>factor_ix, rhs_ixE<gt>>.
A RHS symbol instance I<inst> is one of the B<instances>
of a symch position if and only if there is some 
is one factoring positions in that symch position.
