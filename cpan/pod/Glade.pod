...
The factor of a factoring position C<E<lt>factoring_ix, factor_ixE<gt>>,
is the factor with index C<factor_ix> in the factoring with index C<factoring_ix>.
A factor is one of the factors of a symch_positioning if and only if it is a factor
of one of its factoring positions.
...
Intuitively, a multifactored stretch is a longest possible input span that does not
contain any synced symch positions.
More formally,
