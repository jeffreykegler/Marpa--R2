...
Perl names are resolved to variables in Perl's namespace,
...
=head1 Resolution to a Perl rule evaluation closure

Ordinarily, the arguments to the rule evaluation closure are the per-parse-tree
variable,
...
the arguments to a rule evaluation closure are formed differently.
If the rule is blessed, its rule evaluation closure will always have
...
=head1 Modifying Perl object actions

[ Merge with next graf. ]
