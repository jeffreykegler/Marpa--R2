...
and the child nodes are the items of the quantified sequence of symbol on the
right hand side.
...
but none of the rules is an empty rule.
...
Also, it is a fatal error to bless the implicit action.
...
For both rules and lexemes,
the implicit blessing is C<::undef>.

=item C<::lhs>

The rule is blessed into
...
in lexical order.
If the rule is nulled, the array will contain only two elements:
