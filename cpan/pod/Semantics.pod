...
Using more than one parse series is an advanced technique,
one which is not useful for most applications.
More details about the use of multiple parse series
can be found in
L<another document|Marpa::R2::Semantics::Phases>.

A parse series is divided into two phases:
...
=item * The recognizer is destroyed, or a new parse series
is begun by calling the C<series_restart()> method.
These cases are unusual, because applications will almost always
...
called, there is no Evalution Phase.

The C<value()> method may be called again during
the Evalution Phase, in which case it will produce the
result of the
...
=item * The Parse Series Setup Subphase

This occurs during during the first call
to the C<value()> method of B<the first> parse series.

=item * The Parse Tree Setup Subphase
...
the evaluation subphases are executed "just works".
Application's whose behavior could depend on the details
include
...
A full description of the subphases of the Evalution
Phase is in
L<a separate
...
L<quantified
rules|Marpa::R2::Scanless::DSL/"Quantified rules">.
