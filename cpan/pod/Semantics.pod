...
Usually the idea is that the string "means" something, that it has a B<semantics>.
...
The node at a branch point, "branches" into zero or more "child nodes".
...
The implicit action for a lexeme is to return the literal substring
of the input that corresponds to the lexeme.
...
=head1 Reserve action names

=head1 Perl identifiers as actions
...
=head2 Resolution to Perl variables
