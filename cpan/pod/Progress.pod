...
early stages in the incremental development of something useful.
...
grammar we'll name numeric assignment and expressions as such,
but the we will just call string expressions, C<E<lt>expressionE<gt>>,
and string assignments
C<E<lt>assignment<gt>>.
It seems like a good idea, but it can cause confusion,
and for the sake of our example we pretend that it did.
...
The parse value differs from the other other debugging aids we'll discuss.
All the other debugging tools we use are always available, no matter
how disastrous the parse.
But if you have a problem parsing, you often won't get a parse value.

So we're not going to rely on having a parse value.
But our luck holds, and here's the dump.
...
We'll assume we don't notice this.

We'll also assume, in for the rest of the debugging, that we didn't see this.
Since we don't always have a parse value, we don't want to rely on it.
...
A note in passing: Marpa shows the string location of the tokens it accepts,
discard and rejects.
...
In our example the current location is location 5.
By default,
L<C<show_progress>|Marpa::R2::Scanless::R/"show_progress">
...
Prefixed to the dotted rule are three fields: "C<F0 @0-5 L1c1-16>".
...
The "C<@0-5>" describes the G1 location of the dotted rule in
...
dotted rules where C<E<lt>numeric expressionE<gt>> is not post-dot symbol.
There are none.

In the progress reports, 
C<E<lt>numeric expressionE<gt>> occurs in only two instances.
Here they are:
...
That's fine, but why no relevant occurrences of C<E<lt>numeric expressionE<gt>>?

We look back at the grammar.
Aside for the rule for the 'C<string>' operator,
C<E<lt>numeric expressionE<gt>> on a RHS in two places.
...
This rule is fine -- it does not cause a prediction of a 
C<E<lt>numeric expressionE<gt>> unless we already have a
another prediction of a C<E<lt>numeric expressionE<gt>>,
and it is not supposed to.
That leaves one rule to look at, and it is one
that was supposed to lead to the prediction of new
...
We have concluded following our main example.
...
Marpa always reports the minimum range that includes all the locations
visited in the dotted rule.
The range is always reported in increasing numeric order,
even when the location at
the end of the range was visited before the string location
...
While it does not happen in our main example for this document,
a dotted rule can appear in the same Earley set
more than once.
...
the completed rule in a right recursion of length 13.
The right recursion is indirect, from 
C<E<lt>plain assignmentE<gt>> to
C<E<lt>expression<gt>>, then to
C<E<lt>assignmentE<gt>>,
and then back again.
...
These are counts,
indicated the number of instances of that dotted rule at
the dotted rule's G1 location.
(In each of this lines, the G1 location is 41.)

Instances of a single rule may appear many times -- hundreds or even thousands.
In grammars with right recursion,
the number of instances
is limited only by the
length of the recursion.
The maximum length of a recursion in Marpa is limited primarily by
the available memory.
(Marpa uses a technique invented by Joop Leo to memoize and optimize right recursions
while parsing, but they are expanded when generating progress reports.)
...
"C<F7 x12 @0...38-41 L1c1-L2c40>" say
that the dotted rule is rule 7, which has 12 instances.
A 12 instances have their dot location at G1 location 41,
...
The input string range is the shortest range that includes
...
Their origins are a G1 locations 8 and 18.
...
L<the grammar's C<rule()> method|Marpa::R2::Scanless::G/"rule">.
...
be fully unfolded, and the number of these grow quadratically with the length
of the recursion.
For a lengthy right recursion, this can have a big effect.
...
Here is the C<progress()> output for the latest Earley set.
