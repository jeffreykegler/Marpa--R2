...
"C<@0-3>" say that this rule began at G1 location 0, and that the
...
The current location is G1 location 5,
...
This will prove helpful -- it will allow us to look at the value
of the parse at that point for debugging purposes.
...
that we are in the process of building another C<expression>,
...
=head2 Backward input string spans

Range of G1 locations never run backwards.
But an application is allowed to go back and forth in the input,
so a range as reported in terms of the input string can run backwards.
If it does, your progress report might report a range in the
input string that runs, say, from line 42, column 7 to line 3, column 0.

Most applications always move forward in the input string,
and if yours is one of them, you don't have to worry about backward
input string ranges.
But if you do unusual things when reading the input,
be aware they will be reflected in the ranges Marpa reports
...
quite frequently a particular dotted rule will appear in the same Earley set
more than once.
When it does, it will look like this:
...
it can be inconvenient to show all the origins in a comma-separated
list.
