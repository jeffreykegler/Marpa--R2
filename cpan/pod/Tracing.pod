...
Is more likely that there will be a parse value if you are
...
=item * Set the C<trace_terminals> recognizer named argument to level 1.

=item * Run C<show_rules> on the SLIF grammar.

=item * Run C<show_symbols> on the SLIF grammar.

=item * Run C<show_progress> on the SLIF recognizer.
