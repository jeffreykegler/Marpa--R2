...
the parse reaching end of string, or
...
of the
L<SLIF's C<exhaustion> recognizer setting|
Marpa::R2::Scanless::R/"exhaustion">.
...
=head Querying parse exhaustion

In the detailed cases discussed next, in many cases
it will not be clear, based on the return from
a reading method, whether exhaustion occurred or not.
In most of these cases, you will not care.
But what if you do?

Those applications which want to know whether
a parse is exhausted or not
can directly query
parse exhaustion status
with
the L<C<exhaustion()> method|/Marpa::R2::Scanless::R/"exhaustion()">.
Even what L<parse exhaustion events|Marpa::R2::Event/"Exhaustion events">
are enabled,
using the C<exhaustion()> method will be preferred,
because it reports both asynchronous and synchronous parse exhaustion,
while exhaustion events only occur in cases of asynchronous parse exhaustion.
...
to treat it as exhaustion-conflicted.
By default, the SLIF assumes that
applications are exhausted-conflicted,
as a "lowest common denominator".
This default behavior usually fits well enough
to not need changing.

For a typical application without events, end of parse is end of string.
In this case exhaustion before end of string is a fatal error,
which is usually what is desired.
On end of string,
unless the application checks,
it will not know whether exhaustion occurred, but typically
it does not care.

If the application uses events to signal end of parse,
...
thrown as an error.
Typically, the applications will soon attempts to continue
the reading of input, and when it does there will be a fatal
error.
An application which wants to know about exhaustion immediately,
either to "fast fail" or for other reasons,
can check
for exhaustion explicitly every time an event triggers.
...
that exhaustion *did* occur at end of parse
may check for exhaustion explicitly.

Some applications go beyond being exhaustion-loving,
and want to use exhaustion to signal the end of parse.
...
Aynchronous exhaustion will be a fatal error.
Synchronous exhaustion
will cause failure at the next read,
unless it happens at end of parse.
By default, exhaustion at end of parse will go unreported
but in a genuinely exhaustion-hating application, the
problem will the parse will fail,
and this will show up during evaluation.

Exhaustion-hating applications may want to be stricter
than this.
These can check for exhaustion explicitly whenever
...
can prevent exception being thrown due to parse exhaustion
by enabling exhaustion events.

=head1 Exhaustion-sensing applications 
