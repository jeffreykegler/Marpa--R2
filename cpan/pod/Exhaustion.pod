...
Regardless of what comes it read next,
the parse will fail.
When this happens, the parse is said to be B<exhausted>.

Parse exhaustion has caused sopme confusion,
which this document is intended to address.
...
Many users are focused on one of these kinds and
are either unaware of the the other kind,
or see it as less relevant.

=head1 Hate and love

In an exhaustion-hating application,
...
In an exhaustion-loving applications
parse exhaustion comes with means parse success.
An toy example of an exhaustion-loving
...
We can call them exhaustion-conflicted.
...
These methods needs to treat exhaustion-hating and exhaustion-loving
languages equally.
They also eed to handle the exhaustion-conflicted ones smoothly.
...
The reading methods return successful when
they trigger a L<SLIF parse event|/Marpa::R2::Event>,
or when they are at the end of the current string.

=item *

If the reading method fails for another reason,
the parse exhaustion is treated as being of lesser
importance,
...
never go on to read more input at
a later location.
For these methods,
every exhaustion location is a return
location.
...
showing that the parse exhaustion is usually either not 
important,
or about to become obvious in another way.
...
It may happen that a application is exhaustion-hating but
...
Typically, an application will get on and either try to evaluate the
parse,
...
and returns
or throws a failure that occurs
at an exhaustion location,
...
=head2 Exhausted-hatin and continuing despite exhaustion
...
go on read more input at a later location,
something which is sure to fail,
this is appropriate.
...
an application succeeds and is at the end of input,
in which case throwing an exception is
the right thing to do.
But some exhaustion-loving applications
do not know the length of their input in advance and
as a specialized technique,
try to use parse exhaustion to detect where the
parse ends.
see below on
L<exhaustion-sensitivity|/"Exhaustion-sensitive applications">.
When it is desirable to use exhaustion as a signal
