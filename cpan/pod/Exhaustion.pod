...
The SLIF behavior for exhaustion-conflicted applications has to be aimed
at a "lowest common denominator".
Since it is also a good idea for a default to be a lowest common
denomiator,
the SLIF's default behaviors assumes that an application
is exhausted-conflicted,
Lowest common denominator or not,
the default behavior usually works well enough
...
that exhaustion *did* occur at EOP
