...
We say that an symbol is acceptable at
a location I<L> if
if
there is a valid parse, according to the grammar,
of some input actual to I<L>.
We say that a symbol instance is recognized at
a location I<L> if its end location is I<L>,
and there is a valid parse, according to the grammar,
of some input actual to I<L>.
We say that a symbol is recognized at
a location I<L> if its end location is I<L>,
and there is a valid parse, according to the grammar,
of some input actual to I<L>.
...
whenever a non-nulled instance of symbol
is recognized at the current location.
(The event is called a "completion"
because the symbol will be the LHS of some rule,
and at the point the recognition of the rule,
and therefore its LHS symbol,
is "complete".)
