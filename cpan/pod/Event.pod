...
is usually used to check for parse exhaustion
in preference to checking the exhaustion event.
The exhaustion event itself is often simply discarded
by the application logic,
and is declared only
to make sure control returns to the application
when parse exhaustion occurs.
