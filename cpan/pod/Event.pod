...
producing nulled trees;
and null derivations may be ambiguous,
producing nulled forests.
All activated nulling events declared
for symbols in nulled trees and forests will
trigger.
...
but no actual symbol instance can start at a location unless a prediction,
...
The trigger location of a post-lexeme event is the current location,
...
is consistent with its behavior as a "rewind".
...
It is declared in a
L<C<:lexeme> pseudo-rule|Marpa::R2::DSL/"Lexeme pseudo-rules">.
...
there may be still be more than one lexeme event at a location.
...
L<This
document's example|/"An example">
is very trivial,
and what it does could be done more simply,
but it serves to illustrate the coding of external
scanning.
...
As described above, the specifics vary by type of event,
but all SLIF parse events trigger based on the input actual
to a specific location.
To the left of that location, Marpa's analysis of the grammar
and the input is 100% accurate.

But no lookahead is done, except in the case of pre-lexeme
events,
and even in that case the lookahead is minimal.
To the right, an event occurs if there is any possible input
which would produce a parse which matches that event.
Once the parse is complete,
and the actual input to the right is taken into account,
it is quite possible that it does not produce any parse
which matches a triggered event.
...
from it
is also considered to be nulled.
In L<the example|/"An example">,
when the 
...
An potential non-nulled instance, which may begin
at location 23.
...
return of success of failure by the
