...
a string is consistent with
the actual input up to location I<L>
if the string's prefix of length I<L> is equal
to the prefix of the actual input of length I<L>.
For brevity,
If a string is consistent with the actual input
up to location I<L>,
we will often say the string is B<actual to L>.
We say that a symbol I<S> is B<acceptable> at
a location I<L>,
if an instance of symbol I<S>
starting at location I<L>
is in a valid parse,
...
is B<recognized> at
a location I<L>
if its end location is I<L>,
and it is in
a valid parse, according to the grammar,
of some string actual to I<L>.
We say that a symbol
is B<recognized> at
a location I<L> if it is the symbol of
a symbol instance at location I<L>.
...
A non-lexeme events may trigger during any of the event-triggering
methods.
A lexeme events will only trigger during calls of the
...
A nulling SLIF parse events can be specifed for any
symbol that is not a lexeme.
...
and the two can trigger together,
...
"C<'exhausted">
(The initial single quote is part of the events name,
and indicates it is a reserved name,
which will not conflict with any user-named event.)

Intuitively, exhaustion parse events are only created when needed
for control to return to the application.
More precisely,
...
some event, other than an exhaustion event,
occurring.

=back

If parse exhaustion is not synchronous,
we say that it is B<asynchronous>.

The name of the exhaustion parse event will
always be "C<'exhausted>".
(The initial single quote is part of the event's name,
and indicates it is a reserved name,
which will not conflict with any user-named event.)

Often an exhaustion event is used
only to make sure control returns to the application
-- the event itself is discarded,
and instead the L<C<$recce->exhausted()> method|/"exhausted()">
is used to check for parse exhaustion.
This is because an application usually
does not care whether
exhaustion is synchronous or asynchronous.
The L<C<exhausted()> method|/"exhausted()">
will report both asynchronous and synchronous
parse exhaustion,
while an exhaustion event only triggers for
...
methods resets the pause span to undefined.
...
These implications are Marpa working as designed,
and I hope the reader will usually come to agree,
as is desirable.
...
is nulled.
(This section usually speaks 
of nulled forests because,
...
The very special nature of nulled symbols motivates their
