...
The location at which an event triggers is the B<event location>.
...
L<pre-lexeme/"Pre-lexeme events">.
...
its trigger location and the event location
...
Left of the event location, Marpa's event are 100% accurate.
...
Because the input that the event depends on
is the actual only up to the event location,
events are B<tentative>.
There is no guarantee that a derivation
matching to the event will wind up in the value of the parse,
or in any of the values of an ambiguous parse.
