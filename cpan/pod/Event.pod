...
For some input and grammar,
we say that a parse is B<valid>
if the parse of the input
is valid according to the grammar.
If a string's prefix of length I<L> is equal
to the prefix of the actual input of length I<L>,
we say that
the string is B<consistent with
the actual input up to location L>.
For brevity,
we will often say
that the string is B<actual to L>.
