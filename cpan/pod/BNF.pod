...
the
L<C<rules>|Marpa::R2::Grammar/"rules">
and the
L<C<start>|Marpa::R2::Grammar/"start">
...
Everything related to G0 grammars is unsupported.
G0 grammars do not exist in the Stuifzand interface.
Lexemes, character classes and quoted strings all implicitly
require a G0 grammar to exist
...
Specifically, here is a list of the unsupported SLIF DSL features.

=over 4

=item * Character classes

These implicitly use the G0 grammar.
...
C<:discard> rules control the behavior of the G0 grammar.
...
Rules with the tilde declarator ("C<~>") defined G0 rules.
