...
grammar is created.
Currently, the only argument that may be changed in C<trace_file_handle>.
...
Currently there are G0 and G1 subgrammars.
...
The display form of a symbol is useable as a name -- it is not necessarily unique,
...
This is the name of the symbol as specified by the user in the DSL
for the grammar, as close to that as conveniently possible.
The display form of a symbol is not useable
as a name -- it is not necessarily unique,
and it is subject to change.
The DSL form may not be defined if the symbol did not appear explicitly in the DSL.
