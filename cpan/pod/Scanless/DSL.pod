...
An event initializer, which is an event
name, optionally followed by an
...
as described in the named event statement
...
initializer.
Event initializers are as described above
L<for named event
statements|/"Named event statement">.
