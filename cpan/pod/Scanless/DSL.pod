...
If Marpa finds a Perl subroutine that name,
...
(Again, for this purpose a Perl references is a kind of Perl scalar.)

=head Resolution to Perl variables

If the action name resolves to a scalar Perl variable,
that is the value of the rule.
One way of looking at it, is to say that the effect is as if the resolution
had been to a subroutine that ignores its arguments and always returns that Perl variable,
