...
The adverb consists of its keyword "C<action>",
...
The adverbs allowed are C<action>,
C<bless>,
C<proper> and
C<separator>.
...
Discard rules indicate that the discarded symbol is a top-level G0
...
The purpose of
the default pseudo-rule is change the defaults for
rule adverbs.
Techncially, it has one RHS alternative, but this must always contain
zero RHS primaries.
...
Currently only the C<action> and C<bless> adverbs
