...
The completion event default statement sets up a symbol
so that an named event occurs for every "symbol completion".
A "symbol completion" named event
...
"complete",
and that therefore it is never a symbol completion
