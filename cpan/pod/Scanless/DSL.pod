...
Marking a token "forgiving" makes it subject to
the "longest acceptable tokens match" (LATM)
discipline.
In this,
...
It is possible to use the "forgiving" adverb on a lexeme by
lexeme basis.  When that is done, the lexemes marked
"forgiving" will match only if acceptable to the G1 grammar,
and the lexemes not marked "forgiving" will match regardless
...
LTM is the default for reasons of backward compatibility.
LTM was the SLIF's original token matching discipline because
it more closely models traditional lexing.
Historical reasons also explain why LATM lexemes
are called "forgiving" -- in the original implementation, an
