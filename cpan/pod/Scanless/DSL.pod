...
The C<forgiving> adverb applies only to lexemes and
is only allowed in 
...
is "forgiving".  The default value is 0,
which indicates that a token is not forgiving.

If a token is forgiving it becomes
an exception to the usually strict
"longest tokens match" discipline.
The usual 
"longest tokens match" discipline
dictates that,
if one or more lexemes are found that
are the longest match,
but none of them are acceptable to G1,
the parse fails.
This failure occurs even if shorter lexemes would have been
acceptable to G1, and would have allowed the parse to
continue.

If all of the longest matching lexemes are "forgiving",
then their rejection by G1 does B<not>,
by itself,
cause the parse to fail.
Instead, Marpa continues
the search for acceptable
or discardable tokens, looking for shorter ones.
Marpa will search for tokens of decreasing length,
until

=over 4

=item * the token length decreases below one, or

=item * a token is found which is acceptable to G1, or

=item * a discardable token is found, or

=item * a unforgiving token is found and rejected.

=back

Marking a token "forgiving" is not without some risk.
The lexing might repeatedly scan a very long token, only to reject it
and back up most of that distance to accept a much shorter token.
If this happens, the time required to parse may become quadratic in the
length of the input.
It is for this reaason that "forgiving" is not the default.
Nonetheless, marking a single token, or a small number of them,
forgiving, can allow a parser to be written more compactly or
elegantly than would be the case otherwise.
