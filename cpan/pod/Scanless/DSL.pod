...
The effect of a rule is sometimes lexical --
...
A rule is a quantified rule if
its RHS consists of a single symbol,
followed by a "quantifier".
A "single symbol" in this context is a symbol name or a character class.
...
An RHS alternative of a G1 prioritized rule, in which the action is for the alternative.

=item *

G1 rules other than prioritized rules, in which case the action is for the rule.
...
Details of the semantics are given in
L<their own document|Marpa::R2::Semantics>.
...
if the C<pause> adverb is not also specified.
...
=head2 Fully qualified Perl names
...
(Note that an action value start with a double colon is always treated as a reserved
...
(For the purpose of Perl's symbol tables, references are scalars.)
If it finds one, the action name is resolved to that Perl variable.

Most commonly,
the action name resolves to a subroutine, and that
subroutine becomes the B<semantic closure>.
If the action name resolves to a scalar Perl variable (and, again, for this purpose
references are scalars), then the effect is as if the resolution
had been to a subroutine that ignores it arguments and always returns that Perl variable.
