...
so that a named event occurs either it is either
completed or nulled.
(For the purpose of named events,
a nulled symbol is not considered "completed",
so that at most one of the two events can occur
for any symbol instance.)
...
Again, a null rule will never cause a completion event.

A "nulled" event occurs whenever a zero-length symbol is
recognized.
Null symbols may derive other null symbols,
and these derivations may be ambiguous.
Ambiguous or not,
all such derivations caused "nulled" events.

A "predicted" event occurs at any location where a symbol would
be accepted by the recognizer.
It can be thought of as occurring when a symbol is "expected".
The expectation may or may not come true, depending on future
input.

As a special case, named events are never generated at location 0,
the start of parsing.
These events would typically be ignored.
When not ignored, they are often special cases.
Since at location 0, there has been no input, the set of
events generated a location 0
will always be constant, depending only on the grammar.
The absence of location 0 named events,
when there is processing required,
can be dealt with as the processing of a fixed set of events
...
If pause values of 
both C<before> and C<after> could apply at a
