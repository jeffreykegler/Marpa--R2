...
The first element of the array is the start of span, and the second is its length.
In situations where the input stream location
corresponding to G1 location is thought of
as a single number,
it will usually be considered to be the end of the span:
...
C<progress()> reports progress at that location.
The G1 location may be negative.
An argument of I<-X>
will be interpreted as location I<N+X+1>, where I<N> is
the current location.
This means that an argument of -1 indicates the current G1 location,
...
The logic for determining the string assumes that the application read forward
smoothly in the range of G1 locations from C<$g1_start> to C<$g1_start+$g1_length>.

If the application jumped around in the input
in the range of G1 locations from C<$g1_start> to C<$g1_start+$g1_length>,
the returned substring may omit
all or portions of the input stream,
or it may include skipped portions of the input stream which were skipped,
or both.
...
The C<range_to_string()> method assumes that
the application
moves continuously forward in the input stream, without jumping
forward or backward.
This may not necessarily be the case --
applications may choose to jump around in the input
arbitrarily, using the C<resume()> method.
When a application does not read continuously forward,
C<range_to_string()>
will produce unspecified results, and this is one reason why
