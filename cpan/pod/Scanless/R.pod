...
based on a physical input stream.
The physical input stream is
a Perl string passed as the first argument
to the L< C<< $slr->read() >> method|/read()> method.
By default, the virtual input stream is exactly the
same as the physical input stream.

But the
SLIF allows applications to move about in the input stream.
An application may skip backward and forward.
An application may also read sections of the physical input stream repeatedly.
If an application uses these techniques,
its virtual input stream will no longer
be the same as the physical input stream.

Virtual input streams
are very useful for some applications,
and absolutely necessary for others.
For example, Perl's here-documents cannot be read correctly
unless an application scan skip forward and backward in its
input.
Necessary as virtual input streams may be, their use
makes the idea of location much more complex.
