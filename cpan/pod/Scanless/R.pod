...
A B<recognizer setting> is a named argument.
Recognizer settings are accepted as named arguments
by the
L<C<new()>|/"Constructor">,
L<C<set()>|/"set()"> and
L<C<series_restart()>|"series_restart()"> methods.
Not every method accepts all recognizer settings.
...
a C<grammar> named argument.
The C<grammar> named argument is not allowed
by any other method.
