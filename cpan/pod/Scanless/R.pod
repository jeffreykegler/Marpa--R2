...
always the same settings, including up to the call ot C<value()>,
it will benefit from this method.

Internally, the C<value()> is registering semantic resolutions
the first time it is called, and is caching the result. But, and
this is the responsability of the programmer, if you know that
semantic resolutions will always produce the same result, this
can be bypassed with the C<registrations()> method, as shown above,
