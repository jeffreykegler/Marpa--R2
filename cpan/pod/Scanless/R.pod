...
The C language's pre-processor instructions requires
...
backward and forward in the input stream,
as well as to
read sections of the physical input stream repeatedly.

Physical input stream location is simply Perl C<pos()> location
in the physical input string.
In this document,
the phrase "input stream location" and
the word "location" refer
to physical input stream location,
unless otherwise specified.
Physical input stream location may be zero,
but is never negative.

=head2 Spans and ranges

In this document, we will refer to ordered sets of
...
But ranges allow the end location to be determined
without mental arithmetic, which is convenient for
discussions in which
fenchpost issues are important.

Negative numbers
refer to a locations relative
to the end of some larger set, so that
-1 refers to the last location of the larger set,
-2 refers to the second-to-last location of the larger set,
etc.
As examples, when talking about substrings of the
physical input stream, the range C<(0, -1)>
refers to the entire input stream;
The range C<(0, -2)> refer to the entire input
stream, except for the last character;
The range C<(-1, -1)> refer to substring which consists
only of the last character;
The range C<(-3, -1)> refers to the last 3 characters:
The range C<(-3, -3)> refers to a substring consisting
...
as location in terms of lexemes,
with the first set of lexemes.
(I speak of lexemes being read in "sets",
because lexemes may be ambiguous.
Most "sets" of lexemes will contain exactly one lexeme.)

As each new set of lexemes is read
G1 location 
increases by one.
Most often,
G1 location will not be the same
as the physical input stream location.
B<G1 length>
...
It is sometimes
useful to find the literal substring of the input which corresponds to a span
of G1 locations.
If an application reads the input monotonically within the G1 span
this presents no complications.

More precisely,
let I<X> and I<Y> are a pair of G1 locations.
Let I<Xs> be the first physical input stream location for I<X>
and let I<Xe> be the last physical input stream location for I<X>.
(The length of the G1 in physical input stream terms would then
be I<Xs - Xe + 1>.)
Also,
let I<Ys> and I<Ye> be, respectively,
the first and last physical input stream location for I<Y>.
...
using the notation of the preceding example,
that I<< Ye < Xs >>,
in which case the range of physical input stream locations,
from the G1 location point of view,
runs backwards.
It is also possible
that I<< Ys < Xe >>
and I<< Xs < Ye >>
in which case the physical input stream
locations for
G1 locations I<X> and I<Y> would overlap.

When physical input stream locations run backwards
or overlap,
it present great difficult for those routines
which try to translate G1 locations into literals.
To describe how the SLIF turns a span of G1 locations
into a literal,
I will again use the notation above,
and consider a G1 range from I<X> to I<Y>.
In the more usual C<start, length> notation,
this span would be I<X, Y-X+1>.
...
If I<< Xe < Ys >>,
the literal returned is the range I<Xs, Ye>,
which is the span I<Xs, Ye-Xs+1>.
...
For
applications which read monotonically,
the above will be exactly what is wanted.
For some
applications which do not read monotonically,
the above may be "close enough".
Other non-monotonic applications
may need to use custom logic to reassemble the input
