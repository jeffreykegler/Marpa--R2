...
When the per-parse argument of the value() method is the
...
Reading Phase.
If, like most applications, your application does
not call
the L<C<series_restart()>|"series_restart()"> method,
this means that,
for each recognizer,
it should only be called after the
call of the C<read()> method,
and before any call of the C<value()> method.
For our applications, that do
use the L<C<series_restart()>|"series_restart()"> method,
the details are discussed in a
L<separate document|Marpa::R2::Phases>.
...
Returns the most recent input stream span for the symbol
