...
Input stream locations are ordered sets,
which can be represented as integers from 0 to I<N> for some
I<N>.
I<N+1> is the stream length.
...
and so forth.
A span with
a negative length is interpreted as if it was the range
...
For dealing with more complcated cases, the exact rules are described in
this section.
...
but G1 locations after locations before and after characters.
...
That is, if I<X> and I<Y> are G1 locations
such that I<<X < Y>>
...
For example, applications usually discard whitespace.
This means that if I<W> is the physical input stream location of
a whitespace character,
there will be no G1 location I<Z> such that
I<s(Z) <= W <= e(Z)>.
