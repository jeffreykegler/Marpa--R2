...
This is useful for parsing the same input with different settings.

The recognizer is always dealing with exactly one parse series.
The L<C<new()>|/"Constructor"> starts the first
(and usually the only) parse series.
To end the current parse series and start a new one,
use the L<C<series_restart()>|"series_restart()"> method.

A parse series may result in no parses or be ambiguous,
so that each parse series has zero or more B<parse trees>,
each with its own B<parse result>.
More details about parse series and parse trees
are in
...
Parse exhaustion is synchronous if it occurs at a point when control
would normally return from SLIF to the application,
either because the parse had reached end of string.
or because some event other than the exhastion event
occurs.
Parse exhaustion is asynchronous if it is not synchronous.

The value must be either "C<fatal>"
or "C<event>".
"C<fatal>" is the default.
If the value is "C<fatal>",
asynchronous exhaustion is treated as an error,
and an exception is thrown.
If the value is "C<event>",
an event with the name "C<'exhausted>"
occurs.
(Note that the initial single quote is part of the events name.)

Often exhaustion event is used
to make sure control returns to the application
and is simply thrown away once that happens.
Parse exhaustion is checked for using
the L<C<$recce->exhausted()> method|/"exhausted()">
instead of the event.
This is because an application usually
does not care whether
exhaustion is synchronous or asynchronous,
and the L<C<exhausted()> method|/"exhausted()">
will report both,
while an exhaustion event only occurs for
...
if and only if they are made before
the first call of the
L<C<< $slr->value() >>|"value()"> method
in that parse series.
...
and an exception is thrown.
If the value is "C<event>",
an event with the name "C<'rejected">
occurs.
(Note that the initial single quote is part of the events name.)
...
and starts another.
Parse series are described L<above|"Parse series">.
...
If an application that wants an explicit recognizer setting to persist
...
and each per-parse argument
must be blessed into the same semantics package.
...
If the
L<pause span|Marpa::R2::Event/"Pause span and lexeme">
is defined,
...
of the physical input stream
that corresponds to the tokens read at the current G1 location.
...
to the end location of the new lexeme.
...
On other unthrown failure, C<lexeme_read()> returns 0.
...
an applications should not rely on the exact value, which is subject to change.
...
document|Marpa::R2::Event/"Types of parse event">.
...
the lexeme that triggers a lexeme event.
SLIF parse events are described in detail in
L<a separate document|Marpa::R2::Event>.

The C<pause_lexeme()> method accepts no arguments.
It returns the current
L<pause lexeme|Marpa::R2::Event/"Pause span and lexeme">.
...
The "pause span" is
described in detail
L<in another document|Marpa::R2::Event/"Pause span and lexeme">.
...
Negative arguments are allowed and are
interpreted as
L<described above|/"Spans">.
For example,
the method call C<< $recce->show_progress(0, -1) >>
will print progress reports for the entire parse.

Locations are G1 locations instead of physical input stream locations,
