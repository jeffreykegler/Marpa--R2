...
Form the physical input stream by appending these 7 character:
"C<NO TEXT>".
...
=head2 Evaluation time

A recognizer is in B<evaluation time> after the
first call of the C<< $recce->value() >> method
for the current parse series.
B<Evaluation time> ends when the recognizer is
destroyed, or when the parse series ends.
...
calls of the L<C<new()>|/"Constructor">,
and of the L<C<series_restart()>|"series_restart()"> methods.
The C<ranking_method> setting is also allowed
in those calls of the
L<C<< $slr->set() >>|"set()"> method
that are not made during
L<evaluation time|/"Evaluation time">.
...
call of the C<value()> method is
the source of the semantics package,
the SLIF will require the per-parse argument
of all subsequent calls to specify the same
semantics package.
Specifically,
it will be a fatal error if any subsequent
C<value()> call does 
not have a per-parse argument;
if that per-parse argument
is not blessed;
or if that per-parse argument is blessed into a different package.
