...
It will be a fatal error if these calls are not consistent --
each call will have to have a per-parse argument,
and each per-parse argument
will have to be blessed into the same semantics package.
