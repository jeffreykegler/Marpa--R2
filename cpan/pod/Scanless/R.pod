...
values for
the C<end>
and C<semantics_package> named arguments.
...
grammar is created.
Currently, the arguments that may be changed are C<end>,
C<max_parses>,
C<semantics_package> and
C<trace_file_handle>.
...
If provided it explicitly specifies the per-parse argument for the
parse tree.
...
If the C<semantics_package> named argument of the SLIF
