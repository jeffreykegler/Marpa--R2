...
Pause span and the pause lexeme are set when an event-triggering
methods triggers a lexeme event.
Pause span is
initially undefined,
and it is reset to undefined at the beginning
of every
L<C<< $slr->read() >>|/"read()">
or L<C<< $slr->resume() >> method|/"resume()">.
