...
whose keys are event names,
and whose value is either 0 or 1.
If the value is 1,
...
The setting will be effect at earleme 0,
before any of the input stream is read.
While the L<C<activate()> method|/"activate()">
can be used to change an event's activation
setting once a recognizer has been constructed,
events at earleme 0
are computed by 
