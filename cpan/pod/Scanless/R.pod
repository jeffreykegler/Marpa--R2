...
The range C<[0 ... -2]> refers to the entire input
stream, except for the last character;
the range C<[-1 ... -1]> refers to
...
But when fence post issues are important,
...
If an event triggered, the current location will be
...
but Marpa also repeated passes over the same input
...
Scanning must always being with a 
initial call to the L<C<read()>|/"read()"> method,
...
as location in terms of sets of lexemes,
with the first set of lexemes being at G1 location 1.
...
The length of the G1 span in physical input stream terms would then
be I<(e(X) - s(X)) + 1>.
...
But in others, the opposite is more true -- parse exhaustion indicates
success.
...
So in the case of balanced parentheses, parse exhaustion means parse
success.
...
Marpa must, of course, handle both kinds of language reasonably.
Much confusion arises when users become used to thinking
in terms of one kind of language or the other,
and who therefore find Marpa's treatment of exhaustion strange.
...
call of the recognizer setting-aware methods.
...
when a new parse series is begin,
...
as a L<span/"Spans"> of the physical input stream.
The second argument is the start location of the span,
and the third argument is its length argument.
The default start location is zero.
The default length is -1.
Negative locations and lengths are 
and intrepreted as L<described above|/"Spans">.
...
The read method will always terminate
if the parse becomes exhausted during a C<read()> call.
If the C<read()> method was about to return for other
reasons, C<read()> will return as it normally would.
This includes returns due to the end of the input string,
and due to SLIF parse events.
On the other hand, if the parse is exhausted at a G1
location where the C<read()> method would not normally
return, the exhaustion is treated as a failure
