...
The recognizer is always in exactly one parse series.
...
It sets every other recognizer setting to its default.
An application that wants a non-default recognizer setting to have effect
...
The span will be interpreted as the section of the input stream
...
Fails and returns 0 if called before parsing.
...
SLIF parse events are described in detail in
L<a separate
document|Marpa::R2::Scanless::Event>.
...
document|Marpa::R2::Scanless::Event/"Types of parse event">.
...
G1 location does not correspond to a input stream span so,
as a special case,
...
what is usually intended is the location
...
C<last_completed()> returns the start G1 location and
...
Except at B<EOVS> (the end of the virtual input stream),
...
in which case the choice is arbitrary.
...
C<show_progress()>, as G1 locations.
...
because physical input stream locations do not necessary
represent a unique point in the parse.
The SLIF recognizer
may visit a physical input stream location many times.
...
that the application reads the input in lexical order and without gaps.
