...
This document is written on the assumption that the low-level NAIF interface
is in use.
The more high-level SLIF is similar,
except that it hides even more of these details from the user.
SLIF methods work in the same way,
and use much of the same code,
as the equivalently named methods of the NAIF recognizer.
Certain methods and named arguments exist in the NAIF for which there are
not corresponding methods or arguments in the SLIF, and this is the source
of the differences.
...
null semantics|Marpa::R2::NAIF::Semantics::Null>.
...
L<A separate
document|Marpa::R2::NAIF::Semantics::Phases>
...
on parse order|Marpa::R2::NAIF::Semantics::Order>.
...
document|Marpa::R2::NAIF::Semantics::Infinite>.
