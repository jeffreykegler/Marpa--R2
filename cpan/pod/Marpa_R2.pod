...
rules.
The symbols shared by G0 and G1
...
Previously we skipped over the named arguments to the SLIF grammar
...
The C<action_object> named argument specified a package that is used
...
As a final note, C<actions_object> named argument
requires the package it specifies to
have a C<new> constructor.
...
All Marpa objects have a base grammar and all objects that share
a base grammar write to it.
All Marpa objects that share a base grammar must be created
...
the newly created Marpa object inherits the base grammar of its parent object.
...
threads by creating base grammars
that are exact
