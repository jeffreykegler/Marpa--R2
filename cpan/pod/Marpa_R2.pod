...
The C<Marpa::R2::Scanless::G::parse()> takes a reference
to a string containing the input stream as its argument,
and one or more hash references, as here.
The named arguments in the hash references are passed on the to recognizer,
as it is being created.
Here, as often,
all we need is
the "C<semantics_package>" named argument,
which tells Marpa in which Perl package to
...
Above we used the C<semantics_package> named argument when
we did the pars.
The C<semantics_package> named argument
