...
L<Marpa::R2::Scanless:G::new|Marpa::R2::Scanless::G/"new">
method in detail.
The first two named arguments,
C<action_object> and C<default_action> deal with the
semantics,
and it's best to delay talking about them until
we get into the semantics, below.

But the C<source> argument is an excellent place to start.
...
The parse that results from repeatedly applying G1 rules to the input
is seen as a tree.
(The G1 rules are called structural because they give this tree
its structure.)
...
that the G1 (structural) rules share with the G0 (lexical)
rules.
The symbols by G0 and G1 those lexemes which are not
discarded.
...
Following tradition,
Marpa assign all the nodes of the tree values,
by starting with the leaves.
Marpa's "leaves" will always be G0 symbols,
...
by quoted string,
their value is that quoted string.
...
symbol is value of the top (or "root") node.
The value of the top node becaomes the
value of the parse,
and this is the parse result value
to which the C<value()> method
returns a reference.

Previously we skipped over the named argument to the SLIF grammar
constructor which dealt with semantics.
...
The C<default_action> named argument indicates the default action.
Often actions are Perl functions, which in this context are
called Perl semantic closures.
The <action_object> named argument specified a package that is used
to find the Perl semantic closures.

In this example the default semantics,
as specified by the C<default_action> named
...
that is, the value corresponding to the first symbol of the rules RHS.
...
As a final note, C<actions_object> requires the package it specifies to
...
All Marpa objects have a base grammar and all objects which share
a base grammar write to it.
All Marpa objects which share a base grammar must created
and used in the same
thread.

All Marpa objects are either grammars,
or are created from other Marpa objects.
When a Marpa object is created from another Marpa object,
the child Marpa object inherits the base grammar of its parent object.
Marpa grammars are their own base grammar.

This restriction may be lifted someday, but
in practice it does not seem onerous.
Note that what cannot be shared between
threads is a base grammar B<object> --
you can use
the same grammar in different
threads by creating base grammar
that are exact
copies of each other,
one base grammar per thread.
