...
features.
For example, users will use need to
...
to examine the values of multiple  parses of an ambiguous parse; and
...
These are Marpa's events.
Events are an advanced feature, and are described
L<elsewhere|Marpa::R2::Scanless::R/"events()">.
In this example,
any premature endings due to events will be caused by stray events --
unexpected events due to a mistake in writing the DSL.

Arguably, this check is not really necessary.
Programming a stray event is not a very likely programming mistake.
There are no events, stray or otherwise, in this example.
...
Much more likely than stray events are ambiguous parses -- parses
where the input can be parsed in two or more ways.
