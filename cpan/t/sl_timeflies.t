...
# This is an example of parsing ambiguous English sentences 
# to annotate them with Penn Treebank's syntactic bracketing tags.
# For details, see http://www.cis.upenn.edu/~treebank/ 
...
for my $data ( 'time flies like an arrow', 'fruit flies like a banana' ) {
    ...
    for my $token ( split /(\s+)/, $data ) {
        if (exists $tags{lc $token} ){
            ...
            $recce->lexeme_complete( $start, length $token ) or die $recce->show_progress();
        }
        $start += length ($token);
    ...
    while ( defined( my $value_ref = $recce->value() ) ) {
        ...
        push @actual, $value;
    } ## end while ( defined( my $value_ref = $recce->value() ) )
