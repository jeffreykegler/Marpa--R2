struct marpa_dstack_s *const event_dstack = slr->t_event_dstack;
const int dstack_length = MARPA_DSTACK_LENGTH (event_dstack);
int index;
for (index = 0; index < dstack_length; index++)
  {
    union marpa_slr_event_s *const marpa_slr_event =
      MARPA_DSTACK_INDEX (event_dstack, index);
