...
int marpa_g_precompute(Marpa_Grammar g)
{
    ...
    struct obstack *obs_precompute = marpa_obs_init;
...
@<Declare |marpa_r_earleme_complete| locals@> =
    const ISYID isy_count = ISY_Count_of_G(g);
    Bit_Vector bv_ok_for_chain = bv_create(isy_count);
    struct obstack* const earleme_complete_obs = marpa_obs_init;
@ @<Destroy |marpa_r_earleme_complete| locals@> =
    bv_free(bv_ok_for_chain);
...
PRIVATE void trigger_events(RECCE r)
{
  ...
  unsigned int min, max, start;
  int eim_ix;
  struct obstack *const trigger_events_obs = marpa_obs_init;
    ...
    Marpa_Earley_Set_ID ordinal_arg)
{
    ...
    @<Fail if recognizer not started@>@;
    {
	struct obstack* const obstack = marpa_obs_init;
