...
=head2 C<$slr->g1_read_lexeme(symbol, value, start, length)>

Add a C<$slr->g1_read_lexeme(symbol, value, start, length)> method,
...
C<$slr->read(42)>,
and read all your custom lexemes with C<$slr->g1_read_lexeme(symbol, value, 0, 42)>.

=head2 The C<break> adverb

In the SLIF DSL, add a C<break> adverb for lexemes.
If C<break> is set, a C<Marpa::R2::Scanless::R::read()> will stop when that lexeme is encountered.
If the value of C<break> is reject, the lexeme will not actually be read and current position will be set to the position where the lexeme started.
If the value of C<break> is accept, the lexeme will be read and current position will be set to the position where the lexeme ended.
If the value of C<break> is skip, the lexeme will not be read and current position will be set to the position where the lexeme ended.
