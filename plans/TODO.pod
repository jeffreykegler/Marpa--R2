...
=head2 ASF's should use orderings, instead of using bocage directly.

That way ranking methods will affect the ASF.

=head22 Allow :start to be optional

=head22 Add pseudo-symbols: :null, :empty -- sugar for empty RHS alternative

=head22 Add pseudo-symbols: :external, :never -- sugar for [^\d\D]
