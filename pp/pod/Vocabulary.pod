...
Terms which are special to
Marpa are defined
as they are introduced
in the other documents.

The definitions in this document apply only to the use
of these terms
...
Experts already familiar with the standard terminology
may find in this document a convenient summary
of Marpa's breaks with tradition.
On the other hand,
...
one of which is distinguished as the default start symbols.
...
The production must be one of whose with a LHS that occurs in the symbol string.
...
=item *
A symbol string may also derive itself in a non-trivial derviation.
Such a derivation is called a B<cycle>.
...
Traditionally, a grammar which is not cycle-free is
considered useless, although Marpa will parse with
such grammars.
...
the null parse.
An slight additonal complication is introduced
when it is take into account that in Marpa
...
for some UNAMBIGUOUS input,
it produces more than one parse result.
