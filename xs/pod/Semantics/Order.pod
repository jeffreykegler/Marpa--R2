...
parse results are received may not be repeated.
An application can expect,
even when the order of parse results is arbitrary,
that all appropriate parse results will be included,
...
parse results, it can reduce or eliminate the ambiguity of a parse,
but it does not necessarily do either.
This is because, at each choice point in the parse trees,
