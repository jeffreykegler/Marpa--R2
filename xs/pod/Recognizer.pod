...
Several of named arguments are not allowed once
evaluation has begun.
Evaluation of a parse series begins with
its first C<value> method call.
As a convenience,
the named arguments
in that first C<value> method call
are treated as having been specified BEFORE
evaluation begins.
This eliminates some of the need
for the recognizer's C<set> method.
...
they often will have no effect.
...
are not valid for all of the C<new>, C<set>
and C<value> methods,
...
the C<new> method.
If a named argument causes
an exception
when used in the wrong method call,
...
Marpa allows the user
to control the order which parse results are returned by
the C<value> method,
and to exclude some parse results from the parse series.
This is done by specifying "C<rule>"
or "C<high_rule_only>" as the C<ranking_method>.
...
Such grammars succeed at exactly the point
